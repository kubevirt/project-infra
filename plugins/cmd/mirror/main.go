...
func gatherOptions() options {
	...
	fs.StringVar(&o.mirrorRegex, "mirror-regex", `^https://storage.cloud.google.com/builddeps/.+`, "Regex which matches the mirror source")
	for _, group := range []flagutil.OptionGroup{&o.github} {
		group.AddFlags(fs)
	}
	fs.Parse(os.Args[1:])
...
func main() {
	...
	server := &Server{
		...
		mirrorRegex: regexp.MustCompile(o.mirrorRegex),
