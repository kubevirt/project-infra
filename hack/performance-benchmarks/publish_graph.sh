...
TARGET_GITHUB_REPO="$GIT_AUTHOR_NAME/ci-performance-benchmarks"
GITHUB_TOKEN_PATH="/tmp/github_token"
...
git commit --signoff -m "Add release $RELEASE_VERSION benchmarks data"
git push "https://$GIT_AUTHOR_NAME@github.com/$TARGET_GITHUB_REPO.git" "$BRANCH"

echo "Creating PR..."
gh pr create --base "main" --head "$GIT_AUTHOR_NAME:$BRANCH" --repo "kubevirt/ci-performance-benchmarks" --title "Add release $RELEASE_VERSION benchmarks data" --body "This PR adds the $RELEASE_VERSION benchmarks data to the repository."
