...
autobump() {
    ...
    (
        podman run -v ${PROJECT_INFRA_ROOT}/:/config:z -v ${GITHUB_TOKEN_PATH}:/etc/github -it us-docker.pkg.dev/k8s-infra-prow/images/generic-autobumper:v20240802-66b115076 --config /config/${relative_config_path} --skip-pullrequest
    ) || true
}
