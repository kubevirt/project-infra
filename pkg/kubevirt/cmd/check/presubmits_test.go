...
var _ = Describe("presubmit checks", func() {
	DescribeTable("validateSigNetworkSkipConformanceEnv",
		func(jobs []config.Presubmit, expectedErrs ...string) {
			errs := validateSigNetworkSkipConformanceEnv(&config.JobConfig{
		...
		Entry("fails when env lane is missing", []config.Presubmit{
			...
			newSigNetworkJob("1.36", true, ""),
		}, "no plain sig-network job has SKIP_CONFORMANCE=true"),
		Entry("fails when smoke version mismatches env lane", []config.Presubmit{
			newSigNetworkJob("1.35", false, "true"),
			newSigNetworkJob("1.36", false, ""),
			newSigNetworkJob("1.36", true, ""),
		}, "sig-network-smoke runs on k8s-1.36, but SKIP_CONFORMANCE=true is set on sig-network k8s-1.35"),
		Entry("fails when multiple env lanes exist", []config.Presubmit{
			newSigNetworkJob("1.35", false, "true"),
			newSigNetworkJob("1.36", false, "true"),
			newSigNetworkJob("1.36", true, ""),
		}, "multiple plain sig-network jobs have SKIP_CONFORMANCE=true"),
		Entry("fails when env var is set to unexpected value", []config.Presubmit{
			newSigNetworkJob("1.36", false, "false"),
			newSigNetworkJob("1.36", true, ""),
		}, "job \"pull-kubevirt-e2e-k8s-1.36-sig-network\" has SKIP_CONFORMANCE=\"false\", expected \"true\""),
		...
		Entry("ignores optional SKIP_CONFORMANCE on another version", []config.Presubmit{
			newSigNetworkJob("1.35", false, ""),
			newSigNetworkJob("1.36", false, "true"),
			newSigNetworkJob("1.36", true, ""),
...
func newSigNetworkJob(version string, smoke bool, skipConformanceEnv string) config.Presubmit {
	...
	if skipConformanceEnv != "" {
		env = append(env, v1.EnvVar{Name: "SKIP_CONFORMANCE", Value: skipConformanceEnv})
	}
